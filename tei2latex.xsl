...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 arabic: arabic numerals; roman: lowercase roman numerals; Roman: uppercase roman numerals; alph: lowercase letters; Alph: uppercase letters -->
  ...
  <xsl:template match="/">
    <xsl:text>\documentclass[</xsl:text><xsl:value-of select="$pageSize"/><xsl:if test="$facingLayout = '1'"><xsl:text>,twoside</xsl:text></xsl:if><xsl:text>]{extbook}&#10;</xsl:text>
		<xsl:text>\usepackage[oldstyle,proportional]{libertine}&#10;</xsl:text>
    ...
    <xsl:text>\usepackage[</xsl:text><xsl:value-of select="$baseFontSize"/><xsl:text>pt]{extsizes}&#10;</xsl:text>
    ...
    <xsl:text>\usepackage{fancyhdr}&#10;</xsl:text>
		<xsl:text>\fancyfoot{}&#10;</xsl:text>
		<xsl:text>\renewcommand{\headrulewidth}{0pt} % remove the header&#10;</xsl:text>
    ...
    <xsl:choose>
      <xsl:when test="$lang1 = 'ancientGreek'">
        <xsl:text>\setmainlanguage[variant=ancient]{greek}&#10;</xsl:text>
      </xsl:when>
      <xsl:otherwise>
    ...
    <!--<xsl:text>\usepackage[series={A,B},noend,nofamiliar,noeledsec,noledgroup]{reledmac}&#10;</xsl:text>-->
		<xsl:text>\usepackage[series={A,B},noend,noeledsec]{reledmac}&#10;</xsl:text>
		<xsl:text>\usepackage[shiftedpstarts]{reledpar}&#10;</xsl:text> 
    <xsl:text>\Xarrangement[A]{paragraph}&#10;</xsl:text>
    <xsl:text>\Xarrangement[B]{paragraph}&#10;</xsl:text>
    <!-- set the space before each series of notes -->
    <xsl:text>% set the space before each series of notes&#10;</xsl:text>
    <xsl:text>\Xbeforenotes[A]{</xsl:text><xsl:value-of select="$spaceBeforeNotesA"/><xsl:text>pt}&#10;</xsl:text>
    <xsl:text>\Xbeforenotes[B]{</xsl:text><xsl:value-of select="$spaceBeforeNotesB"/><xsl:text>pt}&#10;</xsl:text>
    <!-- Prevents the lemma from having the same characteristics as in the text (bold, italics, etc.) -->
    <xsl:text>\Xlemmadisablefontselection[A] % Prevents the lemma from having the same characteristics as in the text (bold, italics, etc.)&#10;</xsl:text>
    ...
    <!-- to number the lines; options can be: page, pstart or section    -->
    <xsl:text>\lineation{</xsl:text><xsl:value-of select="$lineationStyle"/><xsl:text>}&#10;</xsl:text> <!-- setting lineation start, and step. -->
    <xsl:text>\setlength{\stanzaindentbase}{20pt}&#10;\setstanzaindents{1,1}&#10;\setcounter{stanzaindentsrepetition}{1}&#10;</xsl:text>
    <xsl:text>\firstlinenum{</xsl:text><xsl:value-of select="$lineationStart"/><xsl:text>}&#10;</xsl:text>
    <xsl:text>\linenumincrement{</xsl:text><xsl:value-of select="$lineationStep"/><xsl:text>}&#10;</xsl:text>
    ...
    <xsl:text>\Xsymlinenum{</xsl:text><xsl:value-of select="$separatorAppEntries"/><xsl:text>}&#10;</xsl:text>
    <!-- order of the critical and familiar footnotes -->
    <xsl:text>\fnpos{critical-familiar}&#10;</xsl:text>
		<xsl:text>\renewcommand*{\thefootnoteA}{\alph{footnoteA}}&#10;</xsl:text>
    <xsl:text>\title{</xsl:text><xsl:value-of select="//tei:teiHeader/tei:fileDesc/tei:titleStmt/tei:title"/><xsl:text>}&#10;</xsl:text>
    <xsl:text>\author{</xsl:text><xsl:value-of select="/tei:TEI/tei:teiHeader/tei:fileDesc/tei:titleStmt/tei:author"/><xsl:text>}&#10;</xsl:text>
    <xsl:text>\date{</xsl:text><xsl:value-of select="/tei:TEI/tei:teiHeader/tei:fileDesc/tei:publicationStmt/tei:date"/><xsl:text>}&#10;</xsl:text>
    ...
    <xsl:text>\sidenotemargin{</xsl:text><xsl:value-of select="$sideNoteLocation"/><xsl:text>}&#10;</xsl:text>
		<xsl:text>\pagenumbering{</xsl:text><xsl:value-of select="$pageNumberingStyle"/><xsl:text>}&#10;</xsl:text>
    ...
    <xsl:if test="$printListWit != '0'">
      <xsl:if test="//tei:listWit">
		...
		<xsl:for-each select="/tei:TEI/tei:text/tei:body/tei:div">
			...
			<xsl:text>\fancyhead[CE]{\textsc{</xsl:text><xsl:value-of select="tei:head"/><xsl:text>}}&#10;</xsl:text>
			<xsl:text>\fancyhead[CO]{\textsc{</xsl:text><xsl:value-of select="/tei:TEI/tei:text/tei:back/tei:div[@xml:id='translation']/tei:div[0+$divNumber]/tei:head"/><xsl:text>}}&#10;</xsl:text>
			...
			<xsl:text>\beginnumbering&#10;</xsl:text>
			<!--<xsl:text>\memorydump&#10;</xsl:text> -->
			<xsl:text>\firstlinenumR{1000000000}&#10;</xsl:text>
    ...
    <xsl:if test="$idxNom = '1'">
      <xsl:if test="$idxNomPrologue != ''">
				<xsl:text>\indexprologue{\small </xsl:text><xsl:value-of select="$idxNomPrologue"/><xsl:text>}&#10;</xsl:text>
			</xsl:if>
      <xsl:text>\printindex[nominum]&#10;</xsl:text>
    </xsl:if>
    <xsl:if test="$idxLoc = '1'">
      <xsl:if test="$idxLocPrologue != ''">
        <xsl:text>\indexprologue{\small </xsl:text><xsl:value-of select="$idxLocPrologue"/><xsl:text>}&#10;</xsl:text>
			</xsl:if>
  ...
  <xsl:template match="tei:l">
	...
	<!-- see tei2latex-cut.xsl for a long template on tei:app, cut from here -->
  ...
  <xsl:template match="tei:note[ancestor::tei:app]">
    <xsl:text>\emph{</xsl:text>
    <xsl:apply-templates/>
    <xsl:text>} </xsl:text>
  </xsl:template>
...
-->
  ...
  <xsl:template name="StringReplace">
	...
	<xsl:template match="tei:milestone[@unit='edPar']">
		<xsl:text>&#10;\pend&#10;\pstart </xsl:text>
	</xsl:template>

	<xsl:template match="tei:w">
		<!--
									not sure why lb matched at et aedificauit tabernacula sibi et seruissuis in die festo hoc
		-->
		<xsl:if test="
	...
	<xsl:template match="tei:milestone[@unit='ch']">
		<xsl:choose>
			<xsl:when test="ancestor::tei:body[@xml:lang='la']">
				<xsl:text>\ledouternote{Cap. </xsl:text>
				<xsl:value-of select="@n"/>
				<xsl:text>}</xsl:text>
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>\ledouternote{Ch. </xsl:text>
				<xsl:value-of select="@n"/>
				<xsl:text>}</xsl:text>
			</xsl:otherwise>
		</xsl:choose>
	...
	<!-- see tei2latex.cut.xsl for a much more elaborate template for tei:app -->
	<xsl:template match="tei:app">
		<xsl:choose>
			<xsl:when test="tei:lem/node() and tei:rdg and tei:note">
				<xsl:text>\edtext{</xsl:text>
				...
				<xsl:text> \emph{sic </xsl:text>
				<xsl:value-of select="substring-after(tei:lem/@wit,'#')"/>
				<xsl:text>}.}}</xsl:text>
			</xsl:when>
			<xsl:when test="tei:lem/node() and tei:note">
				<xsl:text>\edtext{</xsl:text>
				<xsl:apply-templates select="tei:lem"/>
				<xsl:text>}{\Afootnote{</xsl:text>
				<xsl:apply-templates select="tei:note"/>
			...
			<xsl:when test="tei:lem/node() and tei:rdg/node()">
				<xsl:text>\edtext{</xsl:text>
				<xsl:apply-templates select="tei:rdg[1]"/>
				<xsl:text>}{\Afootnote{\emph{</xsl:text>
				<xsl:apply-templates select="id(substring-after(tei:rdg[1]/@source,'#'))/tei:persName/tei:surname"/>
				<xsl:text>} : </xsl:text>
				<xsl:apply-templates select="tei:lem"/>
				<xsl:text> \emph{</xsl:text>
				<xsl:value-of select="substring-after(tei:lem/@wit,'#')"/>
			...
			<xsl:otherwise>
				<!-- might need to add a space or something -->
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
